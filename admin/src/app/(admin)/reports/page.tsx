    ...
    <div className="w-full flex flex-col gap-3.5">
      ...
      <div className="bg-surface-bright rounded-xl border border-subtle p-3.5 md:p-4 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div>
          ...
          <p className="font-body-sm text-body-sm text-on-surface-variant flex items-center gap-2 mt-0.5">
            ...
            <span className="flex items-center gap-1 text-[10px] font-bold"><span className="w-1.5 h-1.5 rounded-full bg-status-success animate-pulse" /> LIVE</span>
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex bg-surface-container-low border border-subtle rounded-lg overflow-hidden p-0.5">
            <input
            ...
            />
            <span className="px-1.5 self-center text-outline text-xs">to</span>
            <input
              className="bg-transparent border-none text-on-surface font-data-mono text-xs focus:ring-0 px-2 py-1 outline-none"
              type="date"
              value={dateTo}
          ...
          <div className="flex gap-1.5">
            <button
              onClick={exportCSV}
              className="bg-surface-container-high hover:bg-surface-bright text-on-surface font-label-caps text-xs font-bold px-3 py-1.5 rounded-lg border border-subtle flex items-center gap-1 transition-colors"
            >
              <span className="material-symbols-outlined text-[16px]">download</span> CSV
            </button>
            <button
              onClick={exportPDF}
              className="bg-secondary text-on-secondary font-label-caps text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 transition-opacity hover:opacity-90 shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="bg-surface-bright p-3.5 md:p-4 rounded-xl border border-subtle shadow-sm flex flex-col justify-between">
          <div>
            <p className="font-label-caps text-[10px] text-on-surface-variant uppercase font-bold mb-0.5">Total Volume</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <span className="font-data-mono text-xl font-bold text-primary">{formatNaira(totalVol)}</span>
              <span className="text-status-success text-[10px] font-bold">{filteredTxns.filter((t: any) => t.status === "completed").length} txns</span>
            </div>
          </div>
          <div className="h-6 mt-3 w-full flex items-end gap-[2px]">
            {hourlyData.map((h, i) => (
        ...
        <div className="bg-surface-bright p-3.5 md:p-4 rounded-xl border border-subtle shadow-sm flex flex-col justify-between">
          <div>
            <p className="font-label-caps text-[10px] text-on-surface-variant uppercase font-bold mb-0.5">Success Rate</p>
            <div className="flex items-baseline gap-2 mb-2 mt-0.5">
              <span className="font-data-mono text-xl font-bold text-secondary">
          ...
          <div className="flex flex-col gap-1.5">
            <div>
              <div className="flex justify-between text-[10px] font-data-mono mb-0.5">
        ...
        <div className="bg-surface-bright p-3.5 md:p-4 rounded-xl border border-subtle shadow-sm flex flex-col justify-between">
          <div>
            <p className="font-label-caps text-[10px] text-on-surface-variant uppercase font-bold mb-0.5">Revenue (Fees)</p>
            <div className="flex flex-col mt-0.5">
              <span className="font-data-mono text-xl font-bold text-tertiary">{formatNaira(totalFees)}</span>
            </div>
          </div>
          <div className="flex gap-3 mt-3 pt-2 border-t border-subtle">
            <div>
              <p className="text-[9px] text-on-surface-variant font-label-caps font-bold">TOTAL VOL</p>
              <p className="font-data-mono text-[10px] font-semibold text-on-surface mt-0.5">{formatNaira(totalVol)}</p>
            </div>
            <div className="border-l border-subtle pl-3">
              <p className="text-[9px] text-on-surface-variant font-label-caps font-bold">PENDING</p>
              <p className="font-data-mono text-[10px] font-semibold text-on-surface mt-0.5">{stats.pendingTxns}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-bright p-3.5 md:p-4 rounded-xl border border-subtle shadow-sm flex flex-col justify-between">
          <div>
            <p className="font-label-caps text-[10px] text-on-surface-variant uppercase font-bold mb-0.5">Active Users (Live)</p>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="font-data-mono text-2xl font-bold text-on-surface tracking-tight">{stats.totalUsers.toLocaleString()}</span>
            </div>
          </div>
          <p className="font-data-mono text-xs font-bold text-status-success mt-3 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3.5">
        <div className="lg:col-span-2 bg-surface-bright border border-subtle rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-3.5 py-2.5 border-b border-subtle bg-surface-container-low flex justify-between items-center">
            ...
            <div className="flex bg-surface-deep rounded-lg p-0.5 gap-1 border border-subtle">
              {(["1H", "24H", "7D", "1M"] as Period[]).map((p) => (
                <button
                  key={p}
                  onClick={() => { setPeriod(p); setDateFrom(""); setDateTo(""); }}
                  className={`px-2.5 py-0.5 text-xs font-bold rounded-md transition-colors ${period === p && !dateFrom ? "bg-primary text-on-primary" : "text-on-surface-variant hover:text-on-surface"}`}
                >
                  {p}
                </button>
              ))}
          ...
          <div className="flex-grow h-48 p-4 relative">
            ...
            <div className="relative w-full h-full flex items-end justify-between px-1 gap-1.5">
              {hourlyData.map((h, i) => (
                <div key={i} className={`w-full ${i % 2 === 0 ? "bg-primary/40" : "bg-secondary/40"} rounded-t-sm`} style={{ height: `${Math.max((h / maxHourly) * 100, 8)}%` }}></div>
              ))}
            </div>
          </div>
          <div className="px-3.5 py-2 bg-surface-container-low border-t border-subtle flex gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-primary"></span>
              <span className="font-body-sm font-semibold">Buy Orders</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-secondary"></span>
              <span className="font-body-sm font-semibold">Sell Orders</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-bright border border-subtle rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-3.5 flex flex-col gap-3 flex-grow justify-between">
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex justify-between font-data-mono text-[10px] text-on-surface-variant font-bold">
                <span>TOTAL: {formatNaira(totalValue)}</span>
                <span className="text-status-success font-bold">LIVE</span>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between p-2.5 bg-surface-container-low rounded-lg border border-subtle">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary"></span>
                  <span className="font-body-sm text-xs font-semibold">Main Wallets</span>
                </div>
                <span className="font-data-mono text-xs font-bold">{mainPct.toFixed(0)}%</span>
              </div>
              <div className="flex items-center justify-between p-2.5 bg-surface-container-low rounded-lg border border-subtle">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-secondary"></span>
                  <span className="font-body-sm text-xs font-semibold">Revenue Vault</span>
                </div>
                <span className="font-data-mono text-xs font-bold">{reservePct.toFixed(0)}%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5">
        <div className="bg-surface-bright border border-subtle rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-3.5 py-2.5 border-b border-subtle bg-surface-container-low flex justify-between items-center">
            ...
            <span className="font-label-caps text-[10px] font-bold text-on-surface-variant">COIN-SPECIFIC DATA</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left font-body-sm">
              <thead className="bg-surface-container-low border-b border-subtle">
                <tr>
                  <th className="px-3 py-2 font-label-caps text-[10px] text-on-surface-variant">ASSET</th>
                  <th className="px-3 py-2 font-label-caps text-[10px] text-on-surface-variant text-right">PRICE (NGN)</th>
                  <th className="px-3 py-2 font-label-caps text-[10px] text-on-surface-variant text-right">24H CHANGE</th>
                  <th className="px-3 py-2 font-label-caps text-[10px] text-on-surface-variant text-right">VOLUME</th>
                </tr>
              </thead>
              <tbody className="font-data-mono divide-y divide-subtle">
                {coins.slice(0, 6).map((coin: any) => (
                  <tr key={coin.id} className="hover:bg-primary/5 transition-colors">
                    <td className="px-3 py-2 text-primary font-bold uppercase text-xs">{coin.symbol}</td>
                    <td className="px-3 py-2 text-right text-xs">{formatNaira(coin.priceNaira || 0)}</td>
                    ...
                    <td className="px-3 py-2 text-right text-on-surface-variant text-xs">{formatUsd(coin.volume24h || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-surface-bright border border-subtle rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-3.5 py-2.5 border-b border-subtle bg-surface-container-low flex justify-between items-center">
            ...
            <span className="font-label-caps text-[10px] font-bold text-on-surface-variant">USER VOLUME RANKING</span>
          </div>
          <div className="flex flex-col divide-y divide-subtle">
            {userVolumes.length === 0 ? (
              ...
              userVolumes.map((e: any, i: number) => (
                <div key={e.id} className="p-2.5 flex items-center justify-between hover:bg-primary/5 transition-colors">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg border border-subtle bg-surface-container-low flex items-center justify-center font-data-mono text-xs font-bold">{String(i + 1).padStart(2, "0")}</div>
                    <div>
                      <p className="font-body-md text-xs font-bold text-on-surface">{e.name}</p>
                      <p className="text-[10px] font-data-mono text-on-surface-variant uppercase">ID: {e.id?.slice(0, 12)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-data-mono text-xs font-bold text-primary">{formatNaira(e.vol)}</p>
                    <p className="text-[9px] font-label-caps text-on-surface-variant uppercase font-bold">{e.txns} TRANSACTIONS</p>
                  </div>
                </div>
              ))
            )}
