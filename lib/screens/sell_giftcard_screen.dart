...
class _SellGiftcardScreenState extends State<SellGiftcardScreen> {
  ...
  void _listenToBrands() {
    _brandsSubscription = FirebaseFirestore.instance.collection('giftcard_brands').snapshots().listen((snap) {
      if (snap.docs.isNotEmpty && mounted) {
        final loaded = snap.docs.map((doc) {
          final d = doc.data();
          return {
            'id': doc.id,
            'name': d['name'] ?? 'Gift Card',
            'icon': Icons.card_giftcard_rounded,
            'color': const Color(0xFF3B82F6),
            'rate': d['rateText'] ?? 'Up to ₦1,200/\$',
            'image': d['imageUrl'] ?? 'https://images.unsplash.com/photo-1621768216002-5ac171876607?q=80&w=400',
          };
        }).toList();
        setState(() => _brands = loaded);
      }
    });
  }
